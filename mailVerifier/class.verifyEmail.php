...
class verifyEmail {
	...
	public function check($email) {
		...
		$responseMessage = $this->_streamResponse();
		...
		switch ($code) {
			...
			case '452':
				 ...
				 */
			case '554':
				/* 554 Email cannot be delivered. Reason: Email detected as Spam by spam filters (because it's empty) */ 
				return TRUE;
			default :
				if(strpos($responseMessage, "550 5.7.1") !== false)
				{
					return TRUE;
				}
				/* 550 5.7.1 Email cannot be delivered. Reason: Domain blacklisted and cannot send email */ 
				return FALSE;
		}
	}
